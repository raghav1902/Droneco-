 ...
 * @description Admin dashboard for tracking analytics, viewing student leads, and configuring courses & questions.
 */
...
const AdminDashboard = () => {
  ...
  // Tab State
  const [activeTab, setActiveTab] = useState('analytics'); // analytics, leads, courses, questions
  ...
  return (
    <AppLayout role="admin" activeTab={activeTab} setActiveTab={setActiveTab}>
          ...
          {loadingStats ? (
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.5rem', marginBottom: '2.5rem' }}>
                {[
                  { label: 'Total Inquiries', val: stats.summary.totalLeads, color: 'var(--accent-primary)' },
                  { label: 'Conversions (Enrolled)', val: stats.summary.enrolledLeads, color: 'var(--success)' },
                  { label: 'Conversion Rate', val: `${stats.summary.conversionRate}%`, color: 'var(--accent-secondary)' },
                  { label: 'Active Follow-ups', val: stats.summary.pendingFollowUps, color: 'var(--warning)' }
                ].map((item, idx) => (
                  <div key={idx} className="glass-card" style={{ borderLeft: `4px solid ${item.color}` }}>
                    <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem', textTransform: 'uppercase', fontWeight: 600 }}>{item.label}</span>
                    <h3 style={{ fontSize: '2.25rem', marginTop: '0.5rem' }}>{item.val}</h3>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', marginBottom: '2.5rem' }}>
                ...
                <div className="glass-card">
                  <h3 style={{ fontSize: '1.1rem', marginBottom: '1.5rem', color: 'var(--text-secondary)' }}>Inquiries by Pipeline Status</h3>
                    ...
                    {stats.leadsByStatus.map((item, index) => {
                      ...
                      return (
                        <div key={item.name} style={{ 
                          padding: '1rem 0',
                          borderBottom: index !== stats.leadsByStatus.length - 1 ? '1px solid var(--border)' : 'none'
                        }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.95rem', marginBottom: '0.6rem' }}>
                          ...
                          <div style={{ width: '100%', height: '6px', background: 'var(--bg-tertiary)', borderRadius: '3px', overflow: 'hidden' }}>
                            <div style={{
                              width: `${percent}%`,
                              height: '100%',
                              background: item.name === 'Enrolled' ? 'var(--success)' : item.name === 'Not Interested' ? 'var(--danger)' : item.name === 'Interested' ? 'var(--accent)' : 'var(--text-muted)',
                              borderRadius: '3px',
                ...
                {/* Leads by Course */}
                <div className="glass-card">
                  <h3 style={{ fontSize: '1.1rem', marginBottom: '1.5rem', color: 'var(--text-secondary)' }}>Inquiries by Course Choice</h3>
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    {stats.leadsByCourse.map((item, index) => {
                      ...
                      return (
                        <div key={item.name} style={{ 
                          padding: '1rem 0',
                          borderBottom: index !== stats.leadsByCourse.length - 1 ? '1px solid var(--border)' : 'none'
                        }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.95rem', marginBottom: '0.6rem' }}>
                            <span style={{ fontWeight: 500 }}>{item.name}</span>
                            <span style={{ fontWeight: 600, color: 'var(--text-secondary)' }}>
      ...
      {/* TAB 2: STUDENT LEADS (Allows Admin to see all user details) */}
      {activeTab === 'leads' && (
        <div className="animate-fade-in">
          {/* Filters */}
